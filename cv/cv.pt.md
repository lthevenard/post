<h1 class="cv-title"> Lucas Thevenard</h2>

<b style="color:#9cc8ff">Afiliação:</b> FGV Direito Rio — Programa de Doutorado em Direito da Regulação

<b style="color:#9cc8ff">Cargo atual:</b> Pesquisador e Professor Assistente

<b style="color:#9cc8ff">Interesses de pesquisa:</b> Participação social na regulação, Análise de Impacto Regulatório (AIR), complexidade regulatória, governança baseada em evidências, estudos empíricos do direito, NLP e métodos computacionais aplicados à regulação

<br>

<h2 class="cv-subtitle-text"> Perfil acadêmico e interesses de pesquisa</h2>

Sou pesquisador e professor universitário, atuando na área do Direito da Regulação. Sou pesquisador visitante do Regulatory Studies Center da George Washington University. No âmbito do Projeto Regulação em Números da FGV, realizo pesquisas com foco na análise empírica de processos regulatórios, especialmente mecanismos de participação social em agências reguladoras, análise de impacto regulatório e agendas regulatórias.

Tenho forte interesse em métodos quantitativos e computacionais aplicados ao direito e às políticas públicas, incluindo estatística, aprendizado de máquina e processamento de linguagem natural (NLP), bem como no uso desses métodos para avaliação de políticas, transparência regulatória e aprendizado institucional.

<br>

<h2 class="cv-subtitle-text"> Formação acadêmica</h2>

- **Doutorado em Direito da Regulação** — FGV Direito Rio  
  <em style="color:#b3b3b3; font-size:90%;">2023–presente (em andamento)</em>

- **Pesquisador visitante no Regulatory Studies Center** — George Washington University  
  <em style="color:#b3b3b3; font-size:90%;">2025–2026</em>

- **Mestrado em Direito da Regulação** — FGV Direito Rio  
  <em style="color:#b3b3b3; font-size:90%;">2021–2022</em>

- **MBA em Relações Internacionais** — FGV CPDOC  
  <em style="color:#b3b3b3; font-size:90%;">2015–2016</em>

- **Bacharelado em Direito** — FGV Direito Rio  
  <em style="color:#b3b3b3; font-size:90%;">2006–2011</em>

- **Aluno de intercâmbio internacional na França** — Institut d'études politiques (Sciences Po)
  <em style="color:#b3b3b3; font-size:90%;">2009</em>

<br>

<h2 class="cv-subtitle-text">Experiência profissional</h2>

- <span style="color:var(--link)"><b>Pesquisador</b> — FGV Direito Rio</span>
  <em style="color:#b3b3b3; font-size:90%;">2020–presente</em> 
  <span style="font-size: 90%">Atuação em pesquisas empíricas em regulação, participação social, ferramentas de melhoria regulatória e análise de políticas públicas, com ênfase em métodos quantitativos e computacionais.</span>

- <span style="color:var(--link)"><b>Professor Assistente</b> — FGV Direito Rio</span>
  <span style="color:var(--link); font-size:90%;">Teoria da Decisão</span>
  <em style="color:#b3b3b3; font-size:90%;">2022–presente</em>
  <span style="font-size: 90%">Responsável pela disciplina **Teoria da Decisão**, com foco em risco, incerteza, tomada de decisão sob informação limitada e aplicações da teoria dos jogos ao direito e à regulação.</span>
  <span style="color:var(--link); font-size:90%;">Programação para Advogados</span>
  <em style="color:#b3b3b3; font-size:90%;">2023–presente</em>
  <span style="font-size: 90%">Responsável, em conjunto com o professor José Luiz Nunes, pela disciplina **Programação para Advogados**, que busca tornar acessíveis a estudantes de direito noções básicas de programação em Python e ciência de dados.</span>

<br>

<h2 class="cv-subtitle-text">Competências</h2>

<b style="color:#9cc8ff; line-height:150%; font-size: 110%">Métodos e ferramentas</b>

- <b style="color:#9cc8ff; line-height:150%; font-size: 95%">Programação e análise de dados:</b>
  - <span style="font-size: 90%;">R (avançado)</span>
  - <span style="font-size: 90%;">Python (avançado)</span>
  - <span style="font-size: 90%;">JavaScript (intermediário)</span>

- <b style="color:#9cc8ff; line-height:150%; font-size: 95%">Análise empírica e métodos quantitativos:</b>
  - <span style="font-size: 90%">Estatística aplicada</span>  
  - <span style="font-size: 90%">Análise de regressão (linear, logística)</span>
  - <span style="font-size: 90%">Análise textual e NLP</span>
  - <span style="font-size: 90%">Aprendizado de máquina supervisionado</span>
  - <span style="font-size: 90%">Visualização de dados</span>

<b style="color:#9cc8ff; line-height:150%; font-size: 110%">Idiomas</b>

- <b style="color:#9cc8ff; font-size: 90%;">Português:</b> nativo  
- <b style="color:#9cc8ff; font-size: 90%;">Inglês:</b> fluente  
- <b style="color:#9cc8ff; font-size: 90%;">Espanhol:</b> avançado  
- <b style="color:#9cc8ff; font-size: 90%;">Francês:</b> avançado
